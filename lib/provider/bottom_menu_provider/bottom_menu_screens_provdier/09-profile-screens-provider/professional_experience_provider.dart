...
class ProfessionalExperienceProvider extends ChangeNotifier {
  ...
  Future<void> addPositionHeld(String name, BuildContext context) async {
    try {
      final response = await NetworkService().postResponse(createOrUpdatePositionsHeld, jsonEncode({'name': name}), true, context, () {});
  ...
  Future<void> addEmploymentHistoryPosition(String name, BuildContext context) async {
    try {
      final response = await NetworkService().postResponse(createOrUpdateEmploymentHistoryPosition, jsonEncode({'name': name}), true, context, () {});
      if (response['statusCode'] == 201 || response['statusCode'] == 200) {
        fetchEmploymentHistoryPositions(context);
      }
    } catch (e) {
